...
public:
    // constructor
    shared_ptr(T* ptr = nullptr) : m_ptr(ptr), m_ref_count(nullptr) {
        if (m_ptr) {
    ...
    // copy constructor
    shared_ptr(const shared_ptr<T>& other) : m_ptr(other.m_ptr), m_ref_count(other.m_ref_count) {
        if (m_ref_count) {
            ++(*m_ref_count);
        }
    }
...
private:
    ...
    size_t* m_ref_count;   // the reference count
};
...
CStrRef::~CStrRef()
{
	if (m_Data != nullptr && --m_Data->m_RefCnt <= 0) 
    	delete m_Data;
}
//-------------------------------------------------------------------------------------------------
