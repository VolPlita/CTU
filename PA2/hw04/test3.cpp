...
public:
    ...
    bool inside(const char * key)
    {
        ...
        if( hash > m_cap || m_table[hash].m_email.get_ptr()== nullptr ) return false;
        if( strcmp(m_table[hash].m_email.get_ptr(), key) == 0 ) return true;
        while (hash < m_cap)
        {
            if( (m_table[hash].m_email.get_ptr() != nullptr) && 
                (strcmp(m_table[hash].m_email.get_ptr(), key) == 0))
                return true;
            hash++;
        }
        return false;
...
private:
    CAccount * m_table;
    size_t m_cap;
    static size_t hashfunc(const char * key)
    {
        size_t sum = 0;
    ...
    void resize(size_t factor)
    {
        size_t new_cap = factor * m_cap + 5;
        auto * new_table = new CAccount[new_cap];
        for (size_t i = 0; i < m_cap; i++) {
            new_table[i] = CAccount(m_table[i]);
        }
        delete [] m_table;
        m_table = new_table;
        m_cap = new_cap;
...
public:
    ...
    CMailIterator(vectorCMail & boxFound) : m_cur(0){m_box = boxFound;}
...
class CMailServer 
{
    CHashTable * m_table;
public:
    CMailServer() : m_table(new CHashTable()) {}
    CMailServer(const CMailServer &src) : m_table(new CHashTable(*src.m_table)) {}
    ~CMailServer() {delete m_table;}

    CMailServer & operator = (const CMailServer & another)
    {
        if (this != &another) 
        {
            CHashTable * new_table = new CHashTable(*another.m_table);
            delete m_table;
            m_table = new_table;
        }
        return *this;
    ...
    {
        if (!m_table->inside(m.get_str_from()))
            m_table->insert(m.get_str_from());
        m_table->find(m.get_str_from()).m_outbox.push_back(m);

        if (!m_table->inside(m.get_str_to()))
            m_table->insert(m.get_str_to());
        m_table->find(m.get_str_to()).m_inbox.push_back(m);
    }

    CMailIterator outbox(const char *email) const 
    {
        if (m_table->inside(email) && m_table->find(email).m_outbox.get_current())
            return CMailIterator(m_table->find(email).m_outbox);
        else return CMailIterator();
    }

    CMailIterator inbox(const char *email) const {
        if (m_table->inside(email) && m_table->find(email).m_inbox.get_current())
            return CMailIterator(m_table->find(email).m_inbox);
        else return CMailIterator();
    }
};
