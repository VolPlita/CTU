...
public:
    ...
    {
        strncpy(m_from.get_ptr(), from, strlen(from) + 1);
        strncpy(m_to.get_ptr(), to, strlen(to) + 1);
        strncpy(m_body.get_ptr(), body, strlen(body) + 1);
    }
    ...
    {
        strncpy(m_from.get_ptr(), another.m_from.get_ptr(), strlen(another.m_from.get_ptr()) + 1);
        strncpy(m_to.get_ptr(), another.m_to.get_ptr(), strlen(another.m_to.get_ptr()) + 1);
        strncpy(m_body.get_ptr(), another.m_body.get_ptr(), strlen(another.m_to.get_ptr()) + 1);
    }
...
public:
    ...
    size_t get_current() const {return current;} // == .size()
    CMail & operator [] (int index) const {return vec[index];}
private:
    T * vec;
    size_t current;
    size_t capacity;
...
private:
    ...
    {
        capacity = factor * capacity + 5;
        auto * newvec = new CMail[capacity]();
        for (size_t i = 0; i < current; i++)
            newvec[i] = CMail(vec.get_ptr()[i]);
        vec = newvec;
    }
public:
    CMailvector(size_t n = 10) 
    {
        vec = new CMail[n];
        current = 0;
        capacity = n;
    }
    CMailvector(const CMailvector & another) 
    {
        capacity = another.capacity;
        current = another.current;
    ...
    {
        if(current == capacity - 2) resize(2);
...
struct CAccount 
{
        ...
        m_email(new char[strlen(another.m_email.get_ptr()) + 1]) 
    {
        ...
        m_inbox = CMailvector(another.m_inbox);
        m_outbox = CMailvector(another.m_outbox);
    }
};
...
private:
    ...
    void resize(){
        size_t newSize = size*2+10;
        auto *newList=new CAccount[newSize]();
        for (size_t i = 0; i < size; i++) {
            newList[i] = CAccount(map[i]);
        }
        delete[]map;
        size=newSize;
        map=newList;
...
public:
    explicit CMap(size_t initSize = 500) {
        map = new CAccount[initSize];
        size = initSize;
    }

    CMap(const CMap &other) {
        size = other.size;
        map = new CAccount[size];
        for (size_t i = 0; i < other.size; i++) {
            if(other.map[i].m_email.get_ptr()!= nullptr)
                map[i] = CAccount(other.map[i]);
        }
    }
    ...
    bool exists(const char* key){
        ...
        if(map[hash].m_email.get_ptr()== nullptr){
            return false;
        }

        if(strcmp(map[hash].m_email.get_ptr(),key)==0){
            return true;
        }
        while (hash<size){
            if(map[hash].m_email.get_ptr()!= nullptr){
    ...
    void insert(const char* key){
        ...
        while (map[hash].m_email.get_ptr()){
            if(hash>size){
                resize();
            }
            hash++;
        }
        map[hash]=CAccount(key);
    }

    CAccount& find(const char *key){
        size_t hash= hasher(key);
        while (strcmp(map[hash].m_email.get_ptr(),key)!=0){
            hash++;
        }
        return map[hash];
    }
    ...
    void print(){
        for(size_t i=0; i<size; i++){
            if(map[i].m_email.get_ptr()!= nullptr){
                cout<<"User: "<<map[i].m_email.get_ptr()<<endl;
            }
        }
    }
...
class CMailIterator 
{
    ...
    size_t m_cur;
public:
    CMailIterator() : m_cur(0) {}
    CMailIterator(CMailvector & x) : m_cur(0) {m_box = x;}
    explicit operator bool() const 
    {
        if (m_cur < m_box.get_current()) return true;
        return false;
    }
    bool operator ! ( void ) const 
    {
        if (m_cur < m_box.get_current()) return false;
        return true;
    }

    const CMail & operator * () const {return m_box[m_cur];}
    ...
    CMail get_current() const {return m_box[m_cur];}
};
...
public:
    ...
    CMailServer(const CMailServer &src) {
        map= new CMap(*src.map);
    }

    CMailServer& operator=(const CMailServer& other){
        if (this != &other) {

            CMap* newMap = new CMap(*other.map);
