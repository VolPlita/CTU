...
void CTile::set_type(int x) {m_type = x;}
...
void CTileMap::populate(CWorldData data)
{
    ...
    for (int x = 0; x < m_len_x; ++x)
    {
        ...
        for (int y = 0; y < m_len_y; ++y)
        {
            row.emplace_back(data.m_types[y + x * MAP_SIZE],
...
void CTileMap::build(int x, int y, int building, CBuildingMap & buildmap, CResources & res_base)
{
    ...
    if (x >= 0 && x < m_len_x && y >= 0 && y < m_len_y && m_map[x][y].get_building() == -1 && m_map[x][y].get_type() != 2)
    {
        switch(building)
        {
            // ADD SETTING UP PARAMETERS FOR BUILDINGS
            case 0:
                if (m_map[x][y].get_resource() == -1)
                {
            ...
            case 2:
                ...
                if (m_map[x][y].get_resource() == 0)
                {
                    m_map[x][y].set_building_id(buildmap.add_building(std::make_shared<CMine>(1, 1, 1, res_base, 10 * 1000, true)));
                    m_map[x][y].set_building(building);
                }
                break;
            case 3:
                if (((x - 1 >= 0 && m_map[x - 1][y].get_type() == 2) || 
                     (x + 1 < m_len_x && m_map[x + 1][y].get_type() == 2) || 
                     (y - 1 >= 0 && m_map[x][y - 1].get_type() == 2) || 
                     (y + 1 < m_len_y && m_map[x][y + 1].get_type() == 2)) && m_map[x][y].get_resource() == -1)
                {
                ...
                break;
            case 4:
                if (m_map[x][y].get_type() == 1 && m_map[x][y].get_resource() == -1)
                {
    ...
    else if (x >= 0 && x < m_len_x && y >= 0 && y < m_len_y && building == -1 && m_map[x][y].get_building() != -1)
    {
...
void CTileMap::remove_tree()
{
    for (int y = 0; y < m_len_y; y++)
    {
        for (int x = 0; x < m_len_x; x++)
        {
            if (m_map[y][x].get_resource() == 2 || m_map[y][x].get_resource() == 3)
                m_map[y][x].set_resource(-1);
        }
    }   
}