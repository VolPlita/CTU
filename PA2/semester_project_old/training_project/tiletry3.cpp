...
class CText
{
    ...
    std::string m_text;
public:
    CText() : renderer(nullptr), m_texture(nullptr), m_font(nullptr) {} // Do smth
    CText(SDL_Renderer * rend, int x, int y, int w, int h, const std::string & text) : renderer(rend), m_texture(nullptr), m_font(nullptr), m_text(text)
    {
        ...
        m_font = TTF_OpenFont("fonts/pixeboy.ttf", 64);
    ...
    ~CText() 
    {
        ...
        // I need to close the font only once
...
public:
    ...
    int get_resource(const string & res)
    {
        ...
        // int res1 = m_resources.find(res)->second;
        // int res2 = m_resources["Gold"];
        // cout << res << endl;
        // cout << res2 << endl;
        //cout << m_resources.find(res)->second << endl;
        // if (m_resources.find(res) != m_resources.end())
...
public:
    ...
    {
        m_gold = CText(renderer, 0, 0, 29 * 5, 64, "Gold:");
        m_stone = CText(renderer, 29 * 5 + 2 * 29, 0, 29 * 6, 64, "Stone:");
        m_wood = CText(renderer, 29 * 5 + 29 * 6 + 4 * 29, 0, 29 * 5, 64, "Wood:");
        m_bread = CText(renderer, 2 * 29 * 5 + 29 * 6 + 6 * 29, 0, 29 * 6, 64, "Bread:");
        m_fish = CText(renderer, 2 * 29 * 5 + 2 * 29 * 6 + 8 * 29, 0, 29 * 5, 64, "Fish:");
        m_population = CText(renderer, 3 * 29 * 5 + 2 * 29 * 6 + 10 * 29, 0, 29 * 11, 64, "Population:");

        m_gold_amount = CText(renderer, 29 * 5, 0, 29, 64, to_string(m_base->get_resource("Gold")));
        m_stone_amount = CText(renderer, 29 * 5 + 2 * 29 + 29 * 6, 0, 29, 64, to_string(m_base->get_resource("Stone")));
        m_wood_amount = CText(renderer, 29 * 5 + 29 * 6 + 4 * 29 + 29 * 5, 0, 29, 64, to_string(m_base->get_resource("Wood")));
        m_bread_amount = CText(renderer, 2 * 29 * 5 + 29 * 6 + 6 * 29 + 29 * 6, 0, 29, 64, to_string(m_base->get_resource("Bread")));
        m_fish_amount = CText(renderer, 2 * 29 * 5 + 2 * 29 * 6 + 8 * 29 + 29 * 5, 0, 29, 64, to_string(m_base->get_resource("Fish")));
        m_population_amount = CText(renderer, 3 * 29 * 5 + 2 * 29 * 6 + 10 * 29 + 29 * 11, 0, 29, 64, to_string(m_base->get_resource("Population")));
    }
    void render_text()
    {
        m_gold.render();
...
int main(int argc, char * argv[])
{
    ...
    CText resourceBar(renderer, TILE_WIDTH, 0, SCREEN_WIDTH - TILE_WIDTH, TILE_WIDTH, "Gold:        Stone:        Wood:        Bread:         Fish:        Population:        ");
    CText goldtext(renderer, TILE_WIDTH * 3 + 16, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Gold")));
    CText stonetext(renderer, TILE_WIDTH * 7, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Stone")));
    CText woodtext(renderer, TILE_WIDTH * 10 + 16, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Wood")));
    CText breadtext(renderer, TILE_WIDTH * 14 - 8, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Bread")));
    CText fishtext(renderer, TILE_WIDTH * 17 + 8, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Fish")));
    CText poptext(renderer, TILE_WIDTH * 23, 0, TILE_WIDTH / 2, TILE_WIDTH, to_string(resourceList.get_resource("Population")));
    //CResourceText resourceBar(&resourceList, renderer);
    //map<int, shared_ptr<ABuilding>> buildingList;
    ...
    while (isRunning)
    {
        ...
        resourceBar.render();
        goldtext.change_text(to_string(resourceList.get_resource("Gold")));
        goldtext.render();
        stonetext.change_text(to_string(resourceList.get_resource("Stone")));
        stonetext.render();
        woodtext.change_text(to_string(resourceList.get_resource("Wood")));
        woodtext.render();
        breadtext.change_text(to_string(resourceList.get_resource("Bread")));
        breadtext.render();
        fishtext.change_text(to_string(resourceList.get_resource("Fish")));
        fishtext.render();
        poptext.change_text(to_string(resourceList.get_resource("Population")));
        poptext.render();
