...
struct CSalary
{
    ...
    CSalary(unsigned salary, unsigned before)
    {
        m_salary = salary;
        m_amount = 1;
        m_before = before;
...
// Utilities
int get_height(CNode * n)
{
    if (n == nullptr)
        return 0;
    return n->height;
}
int get_height(CSalary * n)
{
    if (n == nullptr)
        return 0;
    return n->height;
...
public:
	...
	bool add ( const string & name, const string & surname, const string & email, unsigned int salary )
	{
        ...
        unsigned before = 0;
        root_salary = insert_node_salary(root_salary, salary, before);
    ...
    bool getRank ( const string & name, const string & surname, int & rankMin, int & rankMax ) const
    {
        ...
        // rm += sum_sal_nodes(tmp_root->lesser);
        rankMin = tmp_root->m_before;
        rankMax = rankMin + tmp_root->m_amount - 1;
...
private:
    ...
    CNode * insert_node_name(CNode * root, shared_ptr<CEmployee>& key) // Create a ptr before using
    {
        ...
        // Update heights of ancestor
        if (root != nullptr)
            root->height = 1 + max(get_height(root->lesser), get_height(root->bigger));
        // Get balance to check if we need to rotate
        int balance = getBalance(root);
        // Cases:
        // Left Left
        if (root->lesser != nullptr && balance > 1 && key->relative_name_pos(*root->lesser->Employee) > 0)
            return rightRotate(root);
        // Right right
        if (root->bigger != nullptr && balance < -1 && key->relative_name_pos(*root->bigger->Employee) < 0)
            return leftRotate(root);
        // Left Right
        if (root->lesser != nullptr && balance > 1 && key->relative_name_pos(*root->lesser->Employee) < 0)
        {
            root->lesser = leftRotate(root->lesser);
            return rightRotate(root);
        }
        // Right Left
        if (root->bigger != nullptr && balance < -1 && key->relative_name_pos(*root->bigger->Employee) > 0)
        {
            root->bigger = rightRotate(root->bigger);
            return leftRotate(root);
        }
    ...
    CNode * insert_node_email(CNode * root, shared_ptr<CEmployee>& key) // Create a ptr before using
    {
        ...
        // Cases:
        // Left Left
        if (root->lesser != nullptr && balance > 1 && key->relative_email_pos(*root->lesser->Employee) > 0)
            return rightRotate(root);
        // Right right
        if (root->bigger != nullptr && balance < -1 && key->relative_email_pos(*root->bigger->Employee) < 0)
            return leftRotate(root);
        // Left Right
        if (root->lesser != nullptr && balance > 1 && key->relative_email_pos(*root->lesser->Employee) < 0)
        {
    ...
    CSalary * insert_node_salary(CSalary * root, unsigned salary, unsigned & before)
    {
        if (root == nullptr)
            return new CSalary(salary, before);
        if (salary == root->m_salary)
        {
            if (root->bigger != nullptr) inc_before(root->bigger);
            root->m_amount += 1;
        }
        else if (salary < root->m_salary)
        {
            root->m_before += 1;
            inc_before(root->bigger);
            root->lesser = insert_node_salary(root->lesser, salary, before);
        }
        else
        {
            // Increment sebia
            before += sum_sal_nodes(root->lesser);
            before += root->m_amount;
            root->bigger = insert_node_salary(root->bigger, salary, before);
        }
        // Update heights of ancestor
        if (root != nullptr)
            root->height = 1 + max(get_height(root->lesser), get_height(root->bigger));
        ...
        // Cases:
        // Left Left
        if (root->lesser != nullptr && balance > 1 && salary < root->lesser->m_salary)
            return rightRotate(root);
        // Right right
        if (root->bigger != nullptr && balance < -1 && salary > root->bigger->m_salary)
            return leftRotate(root);
        // Left Right
        if (root->lesser != nullptr && balance > 1 && salary > root->lesser->m_salary)
        {
    ...
    {
        unsigned amount_to_dec = find_salary(root, salary)->m_amount;
        if (root == nullptr)
            return root;
        else if (salary < root->m_salary)
        {
            root->m_before -= amount_to_dec;
            dec_before(root->bigger, amount_to_dec);
        ...
        {
            if (root->bigger != nullptr) dec_before(root->bigger, amount_to_dec);
        ...
        // Cases:
        // Left left
        if (balance > 1 && getBalance(root->lesser) >= 0)
            return rightRotate(root);
        // Right right
        if (balance < -1 && getBalance(root->bigger) <= 0)
            return leftRotate(root);
        // Left right
        if (balance > 1 && getBalance(root->lesser) < 0)
        {
    ...
    //-----
    void inc_before(CSalary * root)
    {
        if (root == nullptr) return;
        root->m_before += 1;
        if (root->lesser != nullptr && root->bigger != nullptr)
        {
            inc_before(root->lesser);
            inc_before(root->bigger);
        }
        if (root->lesser != nullptr && root->bigger == nullptr)
            inc_before(root->lesser);
    ...
    void dec_before(CSalary * root, unsigned amount_to_dec)
    {
        if (root == nullptr) return;
        root->m_before -= amount_to_dec;
        if (root->lesser != nullptr && root->bigger != nullptr)
        {
            dec_before(root->lesser, amount_to_dec);
            dec_before(root->bigger, amount_to_dec);
        }
        if (root->lesser != nullptr && root->bigger == nullptr)
            dec_before(root->lesser, amount_to_dec);
    ...
    CNode * rightRotate (CNode * y) 
    {
        ...
        y->lesser = T2;
        // Update heights
        y->height = max(get_height(y->lesser), get_height(y->bigger)) + 1;
        x->height = max(get_height(x->lesser), get_height(x->bigger)) + 1;
        return x;
    }
    CSalary * rightRotate (CSalary * y)
    {
        CSalary * x = y->lesser;
        CSalary * T2 = x->bigger;
        // Do rotation
    ...
    int getBalance(CNode * n)
    {
        if (n == nullptr) return 0;
        return get_height(n->lesser) - get_height(n->bigger);
    }
    int getBalance(CSalary * n)
    {
        if (n == nullptr) return 0;
        return get_height(n->lesser) - get_height(n->bigger);
    }
...
int main ( void )
{
  ...
  assert (b4.getRank("a12", lo, hi) && lo == 7 && hi == 7);
  assert (b4.getRank("a7", lo, hi) && lo == 8 && hi == 11);
  assert (b4.getRank("a13", lo, hi) && lo == 12 && hi == 12);
  assert (b4.getRank("a3", lo, hi) && lo == 1 && hi == 3);
  assert (b4.getRank("a11", lo, hi) && lo == 4 && hi == 4);
