...
public:
	...
	bool add ( const string & name, const string & surname, const string & email, unsigned int salary )
	{
        ...
        root_name = insert_node_name(root_name, new_employee, nullptr);
    ...
    bool changeName ( const string & email, const string & newName, const string & newSurname )
    {
        ...
        root_name = insert_node_name(root_name, tmp->Employee, nullptr);
    ...
    bool getNext ( const string & name, const string & surname, string & outName, string & outSurname ) const
	{
		...
		CNode * look = find_name(root_name, to_be_found);
        if (look == nullptr) return false;
        if (look->bigger != nullptr)
        {
            // Min in bigger!!!!
            CNode * find = min_node(look->bigger);
            outName = find->Employee->get_name();
            outSurname = find->Employee->get_surname();
        }
        else if (look->prev != nullptr && look->prev->lesser == look)
        {
            outName = look->prev->Employee->get_name();
            outSurname = look->prev->Employee->get_surname();
        }
        else return false;
        return true;
	}
private:
    ...
    CNode * insert_node_name(CNode * root, shared_ptr<CEmployee>& key, CNode * prev) // Create a ptr before using
    {
        if (root == nullptr)
        {
        ...
        if (res > 0)
            root->lesser = insert_node_name(root->lesser, key, root);
        else if (res < 0)
            root->bigger = insert_node_name(root->bigger, key, root);
