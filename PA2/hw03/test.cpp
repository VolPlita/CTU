...
class CRangeList
{
  public:
	...
	CRangeList & operator += (CRange interval)
	{
		...
		{
			//if (bound_start == m_list.end()) {m_list.push_back(interval); return *this;} // I dont like this
		...
		else
		{
			...
			if (bound_start == m_list.end()) return *this; // This may never happen?
			long long dist = distance(bound_start, bound_end);
			if (bound_end == m_list.end()) dist -= 1;
			auto it = ++bound_start;
			if (it == m_list.end()) return *this;
			for (long long i = 0; i < dist; ++i)
			{
				if (it != m_list.end()) it = m_list.erase(it);
			}
    ...
    CRangeList & operator -= (CRange interval)
	{
		...
		else
		{
			...
			else if (bound_start->in_bounds(interval.get_start()))
			{
				...
				if (bound_start == m_list.end()) return *this; // This may never happen?
				long long dist = distance(bound_start, bound_end) - 1;
				auto it = ++bound_start;
				if (it == m_list.end()) return *this;
				for (long long i = 0; i < dist; ++i)
				{
				...
				// if (bound_start == m_list.end()) return *this;
				// auto it = ++bound_start;
				// while (it != m_list.end() && it->direction(interval.get_end()) > 0) // I don't like this
				// 	it = m_list.erase(it);
			}
			else
			{
