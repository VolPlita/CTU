...
public:
	...
	bool in_bounds(long long val) const
	{
        ...
        long long s, e;
        if (m_start == LLONG_MIN) s = m_start;
        else s = m_start - 1;
        if (m_end == LLONG_MAX) e = m_end;
        else e = m_end + 1;
		return (val >= s && val <= e);
	}
	bool in_bounds(CRange interval) const // Shows if another is in bounds of this
	{
		...
		return (interval.get_start() >= s && interval.get_end() <= e);
	}
...
class CRangeList
{
  public:
    ...
    CRangeList & operator -= (CRange interval)
	{
        ...
        else if (interval.get_start() <= bound_start->get_end()) mod_start = true;
...
{
	stream << "{";
	for (long long i = 0, endit = list.m_list.size(); i < endit; ++i)
	{
		if (list.m_list[i].get_start() != list.m_list[i].get_end())
			stream << "<" << list.m_list[i].get_start() << ".." << list.m_list[i].get_end() << ">";
		else 
			stream << list.m_list[i].get_start();
		if (i != endit - 1) stream << ",";
	}
	stream << "}";
	return stream;
