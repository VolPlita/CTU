...
public:
	...
	{
		if (m_start == LLONG_MIN || m_end == LLONG_MAX) return true; // HMMM
		return (val >= (m_start - 1) && val <= (m_end + 1));
	}
	bool in_bounds(CRange interval) const // Shows if another is in bounds of this
	{
		if (m_start == LLONG_MIN || m_end == LLONG_MAX) return true; // HMMMMM
...
class CRangeList
{
  public:
    ...
    CRangeList & operator -= (CRange interval)
	{
        ...
        if (bound_start->get_start() == interval.get_start() && interval.get_end() == bound_start->get_end())
        {
            m_list.erase(bound_start);
            return *this;
        }
        else if (bound_start->get_start() < interval.get_start() && interval.get_end() < bound_start->get_end())
        {
            long long end = bound_start->get_end();
            bound_start->set_end(interval.get_start() - 1);
            m_list.insert(bound_start + 1, CRange(interval.get_end() + 1, end));
            return *this;
        }
        else if (bound_start->get_start() == interval.get_start() && interval.get_end() < bound_start->get_end())
        {
        ...
        else if (interval.get_end() == bound_start->get_end() && bound_start->get_start() < interval.get_start()){
            bound_start->set_end(interval.get_start() - 1);
            return *this;
        }

        if (bound_start->get_start() > interval.get_end()) return *this;
        ...
        if ((bound_start + 1) != m_list.end()) bound_start += 1;
        while(bound_start->get_end() < interval.get_end())
        {
            if (bound_start == m_list.end()) return *this;
            m_list.erase(bound_start);
        }
        if (bound_start->get_start() <= interval.get_end())
        {
            if (bound_start == m_list.end()) return *this;
            bound_start->set_start(interval.get_end() + 1);
        }
