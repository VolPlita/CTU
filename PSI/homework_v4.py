...
def main():
    ...
    while True:
        ...
        try:
            ...
            ok, state, coord_x, coord_y = s_move_to_x(c, states, x_bypass_left, x_bypass_right)
            if not ok:
                break
            ...
            if int(coord_y) != 0:
                ok, state, coord_x, coord_y = s_move_to_x(c, states, down_bypass, up_bypass)
                if not ok:
                    break
...
def s_bypass_left(c, states, state, coord_x, coord_y):
    ...
    if not check_coord_message(coord_message):
    ...
    if int(coord_x) < 0:
        ...
        while int(coord_x) != 0:
            ...
            if int(coord_x) == int(prev_x) and int(coord_y) == int(prev_y):
                s_bypass_left(c, states, state, coord_x, coord_y)
        ...
        while int(coord_x) != 0:
            ...
            coord_x, coord_y = return_coord_message(coord_message)
            if int(coord_x) == int(prev_x) and int(coord_y) == int(prev_y):
                s_bypass_right(c, states, state, coord_x, coord_y)
...
def down_bypass(c, states, state, coord_x, coord_y):
...
def up_bypass(c, states, state, coord_x, coord_y):
    # We need to look right
    while state != 1:
        c.send("104 TURN RIGHT\a\b".encode())
        coord_message = receiver.get_next_message(c)
...
def s_move_to_x(c, states, left_bypass, right_bypass):
    ...
    if int(coord_y) > 0:
        ...
        while int(coord_y) != 0:
            ...
            if int(coord_x) == int(prev_x) and int(coord_y) == int(prev_y):
                if int(coord_x) < 0:
                    left_bypass(c, states, state, coord_x, coord_y)
                else:
                    right_bypass(c, states, state, coord_x, coord_y)
